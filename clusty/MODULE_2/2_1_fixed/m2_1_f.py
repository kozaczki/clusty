...
av_distance = functions.get_average_distance(dataset_folder,100,borders_array)
...
for numbers_chunk in functions.chunker(list(range(1,(number_of_structures+1))),cores):
    ids = [functions.structure_to_neigbours.remote(os.path.join(dataset_folder,functions.prepare_file_name(str(number))),radius_cutoff,number) for number in numbers_chunk]
...
neighbours_max = functions.get_max(final_list)

neighbours = np.zeros((number_of_structures,number_of_beads_per_structure,neighbours_max),dtype = int)

print(str(neighbours.shape) + '\n')

print("building neighbours matrix\n")
...
for x in range(number_of_structures):
    ...
    padded_array = np.zeros((number_of_beads_per_structure, neighbours_max))
    padded_array[:shape[0],:shape[1]] = structure_x_dense
    neighbours[x] = padded_array

print("building neighbours matrix finished\n")

# here goes to indexes

print("building frequencies matrix\n")
...
for x in range(number_of_beads_per_structure):
    ...
    bead_x_all_structures = neighbours[:,x,:]
...
print("building frequencies matrix finished\n")
...
print("building binary matrix finished\n")


print("converting neighbours matrix to products of primes\n")
...
print("converting neighbours matrix to products of primes finished\n")
